...
function ttif:ApplyWorkaroundForFirstMouseover(self, isAura, source, link, linkType, id, rank)
  ...
  local reapplyTooltipModificationFn = function(tooltip)
    ...
    if (linkType == "spell") then
      LinkTypeFuncs.spell(tooltip, tooltip.ttWorkaroundForFirstMouseoverIsAura,
        tooltip.ttWorkaroundForFirstMouseoverSource, tooltip.ttWorkaroundForFirstMouseoverLink,
        tooltip.ttWorkaroundForFirstMouseoverLinkType, tooltip.ttWorkaroundForFirstMouseoverID)
    elseif (linkType == "azessence") then
      LinkTypeFuncs.azessence(tooltip, tooltip.ttWorkaroundForFirstMouseoverLink,
        tooltip.ttWorkaroundForFirstMouseoverLinkType, tooltip.ttWorkaroundForFirstMouseoverID,
        tooltip.ttWorkaroundForFirstMouseoverRank)
    else
      LinkTypeFuncs.item(tooltip, tooltip.ttWorkaroundForFirstMouseoverLink,
        tooltip.ttWorkaroundForFirstMouseoverLinkType, tooltip.ttWorkaroundForFirstMouseoverID)
    end
  end
  ...
  if (not tooltip.ttWorkaroundForFirstMouseoverStatus) then   -- nil = uninitialized
    if not tooltip.SetHook then return end

    tooltip:SetHook("OnTooltipCleared", function(tooltip)
    ...
    tooltip:SetHook("OnUpdate", function(tooltip)
    ...
    tooltip:SetHook("OnHide", function(tooltip)
      resetVarsFn(tooltip); -- 0 = initialized
    end)
...
local function SetCompanionPet_Hook(self, petID)
  if (cfg.if_enable) and (not tipDataAdded[self]) then
    local speciesID, customName, level, xp, maxXp, displayID, isFavorite, name, icon, petType, creatureID, tooltipSource, tooltipDescription, isWild, canBattle, isTradeable, isUnique, obtainable =
    ...
    if (speciesID) then
      ...
      LinkTypeFuncs.battlepet(self, nil, "battlepet", speciesID, level, breedQuality and breedQuality - 1 or nil,
        maxHealth, power, speed, petID, displayID, canBattle)
    end
  end
end
...
local function SetAction_Hook(self, slot)
  if (cfg.if_enable) and (not tipDataAdded[self]) then
        ...
        if (numPetSpells) then
          for i = 1, numPetSpells do
            ...
            if (spellBookItemInfo.itemType == LFF_SPELLBOOK_ITEM_TYPE.PetAction) then
              local spellName, spellSubName, spellID = LibFroznFunctions:GetSpellBookItemName(i,
                LFF_BOOKTYPE_PET_OR_SPELLBANK_PET)
    ...
    elseif (actionType == "summonpet") then
      local speciesID, customName, level, xp, maxXp, displayID, isFavorite, name, icon, petType, creatureID, tooltipSource, tooltipDescription, isWild, canBattle, isTradeable, isUnique, obtainable =
      C_PetJournal.GetPetInfoByPetID(id)
      if (speciesID) then
        local health, maxHealth, power, speed, breedQuality
        if (C_PetJournal.GetPetStats) then -- C_PetJournal.GetPetStats() isn't available in wotlkc 3.4.3 where collections interface has been introduced
        ...
        tipDataAdded[self] = "battlepet"
        LinkTypeFuncs.battlepet(self, nil, "battlepet", speciesID, level, breedQuality and breedQuality - 1 or nil,
          maxHealth, power, speed, id, displayID, canBattle)
      end
    ...
    elseif (actionType == "macro") then
      ...
      elseif (subType == "item") then
        ...
        local name = line and (line:GetText() or "")
        if (name ~= "") then
          local _, link = C_Item.GetInfo(name)
          if (link) then
            local linkType, itemID = link:match("H?(%a+):(%d+)")
            if (itemID) then
              tipDataAdded[self] = linkType
...
local function SetQuestItem_Hook(self, _type, index)
  if (cfg.if_enable) and (not tipDataAdded[self]) then
    ...
    if (itemID) then
      local itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice, classID, subClassID, bindType, expansionID, setID, isCraftingReagent =
...
local function SetQuestLogItem_Hook(self, _type, index)
  if (cfg.if_enable) and (not tipDataAdded[self]) then
    ...
    if (itemID) then
      local itemName, itemLink, itemRarity, _itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice, classID, subClassID, bindType, expansionID, setID, isCraftingReagent =
      C_Item.GetItemInfo(itemID)
      if (itemLink) then
...
local function SetQuestCurrency_Hook(self, _type, index)
  if (cfg.if_enable) and (not tipDataAdded[self]) then
    ...
    local link = C_CurrencyInfo_GetCurrencyLink(currencyID,
      questRewardCurrencyInfo and questRewardCurrencyInfo.totalRewardAmount)
    if (link) then
...
local function SetQuestLogCurrency_Hook(self, _type, index, questID)
  if (cfg.if_enable) and (not tipDataAdded[self]) then
    ...
    if (questRewardCurrencyInfo) then
      local link = C_CurrencyInfo_GetCurrencyLink(questRewardCurrencyInfo.currencyID,
        questRewardCurrencyInfo.totalRewardAmount)
      if (link) then
        local linkType, _currencyID, _quantity = link:match("H?(%a+):(%d+):(%d+)")
        if (_currencyID) then
          tipDataAdded[gtt] = linkType
...
local function SetLFGDungeonReward_Hook(self, dungeonID, rewardIndex)
  if (cfg.if_enable) and (not tipDataAdded[self]) and (self:IsShown()) then
    local name, texture, numItems, isBonusReward, rewardType, rewardID, quality = GetLFGDungeonRewardInfo(dungeonID,
      rewardIndex);                                                                                                              -- see LFGDungeonReadyDialogReward_OnEnter in "LFGFrame.lua"
    if (rewardID) then
      if (rewardType == "item") then
        local itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice, classID, subClassID, bindType, expansionID, setID, isCraftingReagent =
        C_Item.GetItemInfo(rewardID)
        if (itemLink) then
...
local function SetLFGDungeonShortageReward_Hook(self, dungeonID, rewardArg, rewardIndex)
  if (cfg.if_enable) and (not tipDataAdded[self]) and (self:IsShown()) then
    local name, texture, numItems, isBonusReward, rewardType, rewardID, quality = GetLFGDungeonShortageRewardInfo(
    dungeonID, rewardArg, rewardIndex);                                                                                                             -- see LFGDungeonReadyDialogReward_OnEnter in "LFGFrame.lua"
    if (rewardID) then
      if (rewardType == "item") then
        local itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, itemSellPrice, classID, subClassID, bindType, expansionID, setID, isCraftingReagent =
        C_Item.GetItemInfo(rewardID)
...
local function BPTT_Show_Hook(speciesID, level, breedQuality, maxHealth, power, speed, customName)
  if (cfg.if_enable) and (not tipDataAdded[bptt]) and (bptt:IsShown()) then
    local speciesName, speciesIcon, petType, creatureID, tooltipSource, tooltipDescription, isWild, canBattle, isTradeable, isUnique, obtainable, displayID =
    C_PetJournal.GetPetInfoBySpeciesID(speciesID)
    tipDataAdded[bptt] = "battlepet"
    LinkTypeFuncs.battlepet(bptt, nil, "battlepet", speciesID, level, breedQuality, maxHealth, power, speed, nil,
      displayID, canBattle)
  end
end
...
local function PBUTT_UpdateForUnit_Hook(self, petOwner, petIndex)
  if (cfg.if_enable) and (not tipDataAdded[pbputt]) and (pbputt:IsShown()) then
    ...
    if (speciesID) then
      ...
      LinkTypeFuncs.battlepet(pbputt, nil, "battlepet", speciesID, level, breedQuality, maxHealth, power, speed, nil,
        displayID, true)
    end
  end
end
...
local function FBP_Show_Hook(speciesID, level, breedQuality, maxHealth, power, speed, customName, petID)
  if (cfg.if_enable) and (not tipDataAdded[fbptt]) and (fbptt:IsShown()) then
    ...
    LinkTypeFuncs.battlepet(fbptt, nil, "battlepet", speciesID, level, breedQuality, maxHealth, power, speed,
      fbptt.battlePetID, displayID, canBattle)
  end
end
...
local function EITT_SetSpellByQuestReward_Hook(self, rewardIndex, questID)
  local targetTooltip = self.Tooltip
  if (cfg.if_enable) and (not tipDataAdded[targetTooltip]) and (targetTooltip:IsShown()) then
    local texture, name, isTradeskillSpell, isSpellLearned, hideSpellLearnText, isBoostSpell, garrFollowerID, genericUnlock, spellID =
  ...
  if (cfg.if_enable) and (not tipDataAdded[gtt]) and (gtt:IsShown()) then
    ...
    if (wardrobeCollectionFrame.tooltipSourceIndex) then
      local sources = CollectionWardrobeUtil.GetSortedAppearanceSources(self.tooltipVisualID,
        itemsCollectionFrame:GetActiveCategory(), itemsCollectionFrame.transmogLocation)
      local index = CollectionWardrobeUtil.GetValidIndexForNumSources(wardrobeCollectionFrame.tooltipSourceIndex,
        #sources)
...
local function AFAOMA_OnEnter_Hook(self)
  if (cfg.if_enable) and (not tipDataAdded[gtt]) then
    ...
    if (parent) then
      ...
      repeat
        local _achievementID, name, points, completed, month, day, year, description, flags, icon, rewardText, isGuild, wasEarnedByMe, earnedBy, isStatistic =
    ...
    if (type(tip) == "table") and (type(tip.GetObjectType) == "function") then
      ...
      if (tip:GetObjectType() == "GameTooltip") then
        ...
        if (tipName == "GameTooltip") then
          ...
          LibFroznFunctions:HookSecureFuncIfExists("EmbeddedItemTooltip_SetSpellByQuestReward",
            EITT_SetSpellByQuestReward_Hook);                                                                                -- removed since WotLKC 3.4.2
          ...
          LibFroznFunctions:HookSecureFuncIfExists("EmbeddedItemTooltip_SetSpellWithTextureByID",
            EITT_SetSpellWithTextureByID_Hook)
...
function ttif:ADDON_LOADED(event, addOnName, containsBindings)
  ...
  if (addOnName == "Blizzard_AchievementUI") or ((addOnName == MOD_NAME) and (LibFroznFunctions:IsAddOnFinishedLoading("Blizzard_AchievementUI")) and (not addOnsLoaded['Blizzard_AchievementUI'])) then
    if (AchievementFrameAchievementsObjectives.GetMiniAchievement) then -- function GetMiniAchievement() doesn't exist in wotlkc
      ...
      hooksecurefunc(AchievementFrameAchievementsObjectives, "GetMiniAchievement", function(self, index)
        local miniAchievement = self:GetElementAtIndex("MiniAchievementTemplate", self.miniAchivements, index,
  ...
  if (addOnName == "Blizzard_Collections") or ((addOnName == MOD_NAME) and (LibFroznFunctions:IsAddOnFinishedLoading("Blizzard_Collections")) and (not addOnsLoaded['Blizzard_Collections'])) then
    ...
    if (WardrobeCollectionFrame) then
      ...
      hooksecurefunc(WardrobeCollectionFrame.ItemsCollectionFrame, "RefreshAppearanceTooltip",
        WCFICF_RefreshAppearanceTooltip_Hook);                                                                                     -- for items (incl. reapply for tabbing through items with same visualID)
      ...
      hooksecurefunc(WardrobeCollectionFrame.ItemsCollectionFrame, "UpdateItems",
        function(self)                                                                        -- reapply if selecting or scrolling
          if (gtt:IsShown()) then
      ...
      if (WardrobeCollectionFrame.SetsCollectionFrame) then
        hooksecurefunc(WardrobeCollectionFrame.SetsCollectionFrame, "RefreshAppearanceTooltip",
          WCFSCF_RefreshAppearanceTooltip_Hook);                                                                                   -- for sets (incl. reapply for tabbing through items with same visualID)
      end
...
function LinkTypeFuncs:item(link, linkType, id)
  ...
  -- Icon
  local showIcon = (not self.IsEmbedded) and (self.ttSetIconTextureAndText) and
  (not cfg.if_smartIcons or SmartIconEvaluation(self, linkType))
      ...
      if (self == ejtt) then
        if (targetTooltip:IsShown()) then
          ...
          for line in self.linePool:EnumerateActive() do
            if (line and (line:GetText() or ""):match(ITEM_LEVEL_PLUS)) then
              local linePredecessorPoint, linePredecessorRelativeTo, linePredecessorRelativePoint, linePredecessorXOfs, linePredecessorYOfs =
              line:GetPoint(1)
              ...
              if (line == self.textLineAnchor) then
                ...
                for successorLine in self.linePool:EnumerateActive() do
                  local successorLinePredecessorPoint, successorLinePredecessorRelativeTo, successorLinePredecessorRelativePoint, successorLinePredecessorXOfs, successorLinePredecessorYOfs =
                  successorLine:GetPoint(1)

                  if (successorLinePredecessorRelativeTo == line) then
                    local successorLineNumPoints = successorLine:GetNumPoints()
                    local successorLineLeftPoint, successorLineLeftRelativeTo, successorLineLeftRelativePoint, successorLineLeftXOfs, successorLineLeftYOfs =
                    successorLine:GetPoint(2)
                    ...
                    if (successorLineNumPoints > 2) then
                      successorLineRightPoint, successorLineRightRelativeTo, successorLineRightRelativePoint, successorLineRightXOfs, successorLineRightYOfs =
                      successorLine:GetPoint(3)
                    end
                    successorLinePredecessorRelativeTo = linePredecessorRelativeTo

                    successorLine:ClearAllPoints()
                    successorLine:SetPoint(successorLinePredecessorPoint, successorLinePredecessorRelativeTo,
                      successorLinePredecessorRelativePoint, successorLinePredecessorXOfs, successorLinePredecessorYOfs)
                    successorLine:SetPoint(successorLineLeftPoint, successorLineLeftRelativeTo,
                      successorLineLeftRelativePoint, successorLineLeftXOfs, successorLineLeftYOfs)
                    if (successorLineNumPoints > 2) then
                      successorLine:SetPoint(successorLineRightPoint, successorLineRightRelativeTo,
                        successorLineRightRelativePoint, successorLineRightXOfs, successorLineRightYOfs)
                    end
                    break
                  end
                end
  ...
  if (showExpansionIcon) or (showExpansionName) then
    ...
    self:AddLine(
    format("Expansion: %s",
      (expansionIconTextureMarkup and expansionIconTextureMarkup or "") ..
      (showExpansionIcon and showExpansionName and " " or "") .. (showExpansionName and expansionName or "")),
      unpack(cfg.if_infoColor))
  end
  ...
  if (showItemEnchantInfo) then
    ...
    if (enchant) and (enchant ~= LFF_ENCHANT.none) and (enchant ~= LFF_ENCHANT.available) then
      self:AddLine(
      format("Enchant Description: %s",
        ((enchant.spellIconID ~= 136235) and (CreateTextureMarkup(enchant.spellIconID, 64, 64, 0, 0, 0.07, 0.93, 0.07, 0.93) .. " ") or "") ..
        GREEN_FONT_COLOR:WrapTextInColorCode(enchant.spellName)), cfg.if_infoColor[1], cfg.if_infoColor[2],
        cfg.if_infoColor[3]);                                                                                                                                                                                                                                                                                      -- 136235 = Samwise Didier Icon aka missing icon
...
function LinkTypeFuncs:keystone(link, linkType, itemID, mapID, keystoneLevel, ...) -- modifierID1, modifierID2, modifierID3, modifierID4
  ...
  if (showId or showRewardLevel or showWeeklyRewardLevel or showTimeLimit or showAffixInfo) then
    ...
    local infoColorMixin = CreateColor(cfg.if_infoColor[1], cfg.if_infoColor[2], cfg.if_infoColor[3],
      (cfg.if_infoColor[4] or 1))
    ...
    if (cfg.if_modifyKeystoneTips) then
      ...
      elseif (showRewardLevel and showWeeklyRewardLevel) then
        textRight2:SetText(infoColorMixin:WrapTextInColorCode(format("RL: %s, WRL: %s", endOfRunRewardLevelText,
          weeklyRewardLevelText)))
      ...
      elseif (showRewardLevel and showWeeklyRewardLevel) then
        self:AddLine(format("RewardLevel: %s, WeeklyRewardLevel: %s", endOfRunRewardLevelText, weeklyRewardLevelText),
          unpack(cfg.if_infoColor))
      elseif (showRewardLevel and not showWeeklyRewardLevel) then
      ...
      for i = 1, select('#', ...) do
        ...
        if (modifierID) then
          ...
          if (modifierName and modifierDescription) then
            self:AddLine(
            format("%s %s", CreateTextureMarkup(fileDataID, 64, 64, 0, 0, 0.07, 0.93, 0.07, 0.93),
              GREEN_FONT_COLOR:WrapTextInColorCode(modifierName)), cfg.if_infoColor[1], cfg.if_infoColor[2],
              cfg.if_infoColor[3])
  ...
  if (showAuraCaster) then
    ...
    if (sourceName) and (sourceName ~= TTIF_UnknownObject and sourceName ~= "") then
      ...
      if (UnitIsPlayer(source)) and (cfg.if_colorAuraCasterByClass) then
        ...
        colorAuraCaster = LibFroznFunctions:GetClassColor(sourceClassID, nil,
          cfg.enableCustomClassColors and TT_ExtendedConfig.customClassColors or nil) or
        CreateColor(unpack(cfg.if_infoColor))
      end
      ...
      if (not colorAuraCaster) and (cfg.if_colorAuraCasterByReaction) then
        ...
        colorAuraCaster = (cfg["colorReactText" .. sourceReactionIndex] and CreateColor(unpack(cfg["colorReactText" .. sourceReactionIndex]))) or
        CreateColor(unpack(cfg.if_infoColor))
      end
    ...
    if (mawPowerID and spellID) then
      ...
      if (rarityAtlas) then
        local rarityAtlasColors = { -- see table UiTextureAtlasElement name "jailerstower-animapowerlist-powerborder*"
          ["jailerstower-animapowerlist-powerborder-white"] = LibFroznFunctions:GetItemQualityColor(LFF_ITEM_QUALITY
          .Common),
          ["jailerstower-animapowerlist-powerborder-green"] = LibFroznFunctions:GetItemQualityColor(LFF_ITEM_QUALITY
          .Uncommon),
          ["jailerstower-animapowerlist-powerborder-blue"] = LibFroznFunctions:GetItemQualityColor(LFF_ITEM_QUALITY.Rare),
          ["jailerstower-animapowerlist-powerborder-purple"] = LibFroznFunctions:GetItemQualityColor(LFF_ITEM_QUALITY
          .Epic)
        }
...
function LinkTypeFuncs:currency(link, linkType, currencyID, quantity)
  ...
  local icon, quality
  local isCurrencyContainer = C_CurrencyInfo.IsCurrencyContainer and
  C_CurrencyInfo.IsCurrencyContainer(currencyID, _quantity)
...
-- achievement
function LinkTypeFuncs:achievement(link, linkType, achievementID, guid, completed, month, day, year, criteria1, criteria2,
                                   criteria3, criteria4)
  local _achievementID, name, points, _completed, _month, _day, _year, description, flags, icon, rewardText, isGuild, wasEarnedByMe, earnedBy, isStatistic =
  GetAchievementInfo(achievementID)
...
-- battle pet
function LinkTypeFuncs:battlepet(link, linkType, speciesID, level, breedQuality, maxHealth, power, speed, petID,
                                 displayID, canBattle)
  local speciesName, speciesIcon, petType, creatureID, tooltipSource, tooltipDescription, isWild, canBattle, isTradeable, isUnique, obtainable, _displayID =
  C_PetJournal.GetPetInfoBySpeciesID(speciesID)
  ...
  if (showLevel or showId) then
    if (showLevel) then
      if (self == bptt or self == fbptt) then
        ...
        local healthTexturePoint, healthTextureRelativeTo, healthTextureRelativePoint, healthTextureXOfs, healthTextureYOfs =
        self.HealthTexture:GetPoint()
        healthTextureRelativeTo = levelRelativeTo

        self.HealthTexture:ClearAllPoints()
        self.HealthTexture:SetPoint(healthTexturePoint, healthTextureRelativeTo, healthTextureRelativePoint,
          healthTextureXOfs, healthTextureYOfs)
...
function LinkTypeFuncs:battlePetAbil(link, linkType, inBattle, abilityID, speciesID, petID, additionalText)
  ...
  if (inBattle) then
    ...
    local _abilityID, _abilityName, _abilityIcon, maxCooldown, unparsedDescription, numTurns, _petType, noStrongWeakHints =
...
function CustomTypeFuncs:runeforgePower(link, linkType, runeforgePowerID)
  ...
  if (cfg.if_runeforgePowerColoredBorder) then
    local runeforgePowerColor = CreateColor(LEGENDARY_ORANGE_COLOR.r, LEGENDARY_ORANGE_COLOR.g, LEGENDARY_ORANGE_COLOR.b,
      1);                                                                                                                   -- see RuneforgePowerBaseMixin:OnEnter() in "RuneforgeUtil.lua"
